...
import service.TypeDeMusiqueService;
...
public class ArtisteBackFXMLController implements Initializable {
    ...
    TypeDeMusiqueService ms = new TypeDeMusiqueService();
    
    @FXML
    private TextField NomArtiste;
    @FXML
    private ComboBox<Type_De_Musique> TypeDeMusique;
    @FXML
    private TableView<Artiste> listeArtiste;
    @FXML
    private TableColumn<Artiste, String> Nom_Artiste;
    @FXML
    private TableColumn<Artiste, Type_De_Musique> Type_De_Musique;
    @FXML
    ...
    public void fillArtisteTV() {
        ...
        Type_De_Musique.setCellValueFactory(new PropertyValueFactory<Artiste, Type_De_Musique>("Type_De_Musique"));
        listeArtiste.setItems(la);
    }
    
    public void addArtisteDB() {
        if ((NomArtiste.getText().equals("")) || (TypeDeMusique
            ...
            Artiste a = new Artiste(NomArtiste.getText(),
                    TypeDeMusique.getSelectionModel().getSelectedItem());
            as.ajouter(a);
            NomArtiste.setText("");
    ...
    private void modifierArtisteDB(){
        if ((NomArtiste.getText().equals("")) || 
                (TypeDeMusique.getSelectionModel().getSelectedItem()) == null){
            SystemTray tray = SystemTray.getSystemTray();
            Image image = Toolkit.getDefaultToolkit().createImage("icon.png");
            TrayIcon trayIcon = new TrayIcon(image, "Tray Demo");
            trayIcon.setImageAutoSize(true);
    ...
    private void fillFields(MouseEvent event) {
        ...
        TypeDeMusique.setValue(selected.getType_De_Musique());
    }
    
    private void fillCBTypeDeMusique() {
        ObservableList<Type_De_Musique> lm = ms.afficher();
        TypeDeMusique.setItems(lm);
    }
}
